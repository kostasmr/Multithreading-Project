...
DB* db_open_ex(const char* basedir, uint64_t cache_size)
{
    ...
    pthread_mutex_init(&(self->general_lock),NULL);         //initialize general lock
    pthread_mutex_init(&(self->readers_lock),NULL);         //initialize readers lock
    self->readers=0;                                        //initialize variable
...
{
    int var;                                            //helper variable to return in the end of the method
    pthread_mutex_lock(&self->general_lock);            //lock for other writers and all readers because we want to write 
    var = db_add(self, key, value);                     //call db_add and pass the returned variable (write)
    pthread_mutex_unlock(&self->general_lock);          //unlock
    return var;
}
...
{
    int var;                                            //helper variable to return in the end of the method
    pthread_mutex_lock(&self->readers_lock);            //lock for other readers 
    self->readers++;                                    //add a reader 
    if(self->readers==1)
    {
        pthread_mutex_lock(&self->general_lock);        //lock for writers too
    }
    pthread_mutex_unlock(&self->readers_lock);          //unlock for readers
    var = db_get(self, key, value);                     //call db_get and pass the returned variable (read)
    pthread_mutex_lock(&self->readers_lock);            //lock for other readers
    self->readers--;                                    //remove a reader
    if(self->readers==0)
    {
        pthread_mutex_unlock(&self->general_lock);      //unlock for writers
    }
    pthread_mutex_unlock(&self->readers_lock);          //unlock for readers
